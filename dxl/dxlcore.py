...
class DxlElement(object):
    ...
    def registerModel(cls,model_number,model_cls):
        if model_number not in list(cls.DxlModels.keys()):
    ...
    def instantiateMotor(cls,id,model_number):
        if not model_number in list(cls.DxlModels.keys()):
    ...
    def getRegisterCmd(self,name):
        if not name in list(self.registers.keys()):
    ...
    def sort(self):
        self.registers = OrderedDict( sorted(iter(self.registers.items()), key=lambda x: x[1].address) )
